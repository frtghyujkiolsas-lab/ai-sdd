...
| 需求 ID | 可观察行为 | 验收 ID | 对应产物 |
| --- | --- | --- | --- |
| REQ-01 | 填写完整员工字段并上传单张照片后可提交生成 | AC-01 | 上传页、`POST /api/generate` |
| REQ-02 | 系统返回统一规格证件照 | AC-02 | 处理页、`id_photo` |
| REQ-03 | 系统返回包含正确字段的工牌 PNG，用户可下载 | AC-03 | 结果页、`badge_png` |
| REQ-04 | 异常情况被识别并呈现可读反馈 | AC-04 | 上传页 / 处理页、`error` |
...
| 需求 ID | 模块 | 说明 |
| --- | --- | --- |
| REQ-01 | 信息填写 | 姓名、部门、工号、职位等基础字段 |
| REQ-01 | 生活照上传 | 支持 JPG / PNG，上传后做基础质量检测 |
| REQ-02 | AI 证件照生成 | 调用 HivisionIDPhotos 或同类能力，完成抠图、背景替换、裁剪 |
| REQ-03 | 工牌模板合成 | 使用 HTML / CSS 模板放入头像和员工信息 |
| REQ-03 | 预览与下载 | 生成工牌预览图，支持下载 PNG |
| REQ-04 | 异常反馈 | 对字段、照片和下游服务异常展示可读提示 |
...
```mermaid
flowchart LR
    A["前端<br/>React / Vite 上传表单 + 预览"] -->|上传照片 + 信息| B["后端<br/>Python HTTP service 调度"]
...
| 组件 | 方案 | 作用 |
| --- | --- | --- |
| 前端 | React + Vite | 上传照片、填写信息、预览工牌 |
| 后端 | Python HTTP service | 接收请求、调用 AI、渲染模板 |
...
| 验收 ID | 标准 |
| --- | --- |
| AC-01 | 合格生活照与完整字段可提交并触发生成流程。 |
| AC-02 | 输出证件照不变形、不换脸，保留本人特征，背景和比例统一。 |
| AC-03 | 工牌模板不跑版，姓名、部门、岗位、工号正确渲染，PNG 可下载。 |
| AC-04 | 缺照片、异常照片或下游服务异常时，页面给出可理解的重试或补充提示。 |
